...
<nav class="navbar not_active ease-in-out duration-300" id="navbar">
  <label for="btn_nav" id="btn_nav_label" class="w-[40px] h-[40px] rounded-full bg-white absolute -right-4 top-4 z-10 select-none cursor-pointer border-4 border-black/5 flex justify-center items-center ease-in-out duration-200">
  ...
  <div class="container py-2 px-4 flex flex-col justify-between items-center h-full w-full">
    <div class="block">
      ...
      <div class="flex flex-col justify-center items-center gap-2 mt-2">
        <?php foreach ($list_menu as $list) : ?>
          <a href="<?= $list['link'] ?>" class="flex justify-start items-center gap-2 rounded-md p-1 hover:bg-black/10 relative ease-in-out duration-300 menu">
    ...
    <div class="block">
      <form action="<?= base_url('logout') ?>" method="post" class="flex justify-start items-center bg-red-600 hover:bg-red-700 rounded-md p-2 gap-1 ease-in-out duration-300 btm_menu relative">
        <input type="hidden" value="0" name="user_token">
